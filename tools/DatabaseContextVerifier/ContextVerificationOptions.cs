...
internal sealed record ContextVerificationOptions(
    string Host,
    int Port,
    string Database,
    string SecretDirectory)
{
    internal static ContextVerificationOptions Parse(string[] args)
    {
        ...
        for (var index = 0; index < args.Length; index += 2)
        {
                ...
                || !args[index].StartsWith("--", StringComparison.Ordinal))
            {
                throw new ArgumentException(
                    "Uso: --host <host> --port <port> --database <db> --secret-directory <ruta>");
            }
        ...
        return new ContextVerificationOptions(
            host,
            port,
            database,
