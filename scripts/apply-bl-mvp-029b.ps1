...
Write-Host "BL-MVP-029B: corrigiendo el falso positivo E2E de no enumeracion..."

$currentBranch = (git branch --show-current).Trim()
Assert-LastExitCode "Consulta de rama Git"
if ($currentBranch -ne "main") {
    throw "BL-MVP-029B debe aplicarse sobre main. Rama actual: '$currentBranch'."
}
...
$testContent = [System.IO.File]::ReadAllText($Target, [System.Text.Encoding]::UTF8)

if ($testContent.Contains(
    "page.getByText(/cuenta limitada|dirección IP|correo existe/i)")) {
    throw "La asercion global defectuosa sigue presente. Extraiga de nuevo BL-MVP-029B."
}
...
if (-not $testContent.Contains(
    ...
    throw "No se encontro la comprobacion no enumerativa acotada al mensaje 429."
}
...
foreach ($relativePath in @(
    ...
    if (-not $baseContent.Contains('"' + $relativePath + '"')) {
        throw "El instalador base no reconoce el artefacto 029B: $relativePath"
    }
}
...
Assert-LastExitCode "TypeScript E2E de BL-MVP-029B"
...
Assert-LastExitCode "Playwright focalizado BL-MVP-029B"
Write-Host "OK: Playwright focalizado BL-MVP-029B aprobado."

git diff --check -- $TargetRelative "scripts/apply-bl-mvp-029.ps1"
Assert-LastExitCode "git diff --check de BL-MVP-029B"
Write-Host "OK: git diff --check aprobado."

Write-Host ""
Write-Host "OK: BL-MVP-029B aplicado y validado."
