...
Write-Host "BL-MVP-031J: acotando la verificacion de EDITOR a la lista de asignaciones..."

$currentBranch = (git branch --show-current).Trim()
Assert-LastExitCode "Consulta de rama Git"
if ($currentBranch -ne "main") {
    throw "BL-MVP-031J debe aplicarse sobre main. Rama actual: '$currentBranch'."
}
...
$appRelative = "apps/web/src/routes/administration/RoleManagementPage.tsx"
$mainInstallerPath = Join-Path $RepoRoot "scripts\apply-bl-mvp-031.ps1"

foreach ($relativePath in @($testRelative, $appRelative, "scripts/apply-bl-mvp-031.ps1")) {
...
$testContent = [System.IO.File]::ReadAllText(
    (Join-Path $RepoRoot $testRelative),
    [System.Text.Encoding]::UTF8)

$ambiguous = "getByText('EDITOR', { exact: true })"
$scoped = "getByLabel('Asignaciones de la cuenta').getByText('EDITOR', { exact: true })"

if (-not $testContent.Contains($scoped)) {
    throw "No se encontro el locator EDITOR acotado a Asignaciones de la cuenta."
}

$standaloneCount = ([regex]::Matches(
    $testContent,
    "page\.getByText\('EDITOR', \{ exact: true \}\)"
)).Count

if ($standaloneCount -gt 0) {
    throw "Sigue presente un locator global ambiguo para EDITOR."
}

$appContent = [System.IO.File]::ReadAllText(
    (Join-Path $RepoRoot $appRelative),
    [System.Text.Encoding]::UTF8)

if (-not $appContent.Contains("result.data.assignment.assignmentId")) {
    throw "No esta presente la actualizacion local canonica introducida por 031H."
}
...
foreach ($artifact in @(
    "INSTRUCCIONES_INSTALACION_MVP_F1_BL-MVP-031J.md",
...
Assert-LastExitCode "Prettier BL-MVP-031J"

& $prettier --check @formatTargets
Assert-LastExitCode "Prettier check BL-MVP-031J"
Write-Host "OK: archivos 031J formateados."

npm.cmd run typecheck
Assert-LastExitCode "TypeScript web BL-MVP-031J"
Write-Host "OK: TypeScript web aprobado."

npm.cmd run build
Assert-LastExitCode "Build frontend BL-MVP-031J"
Write-Host "OK: build frontend actualizado antes de Playwright."

npm.cmd run typecheck:e2e
Assert-LastExitCode "TypeScript E2E BL-MVP-031J"
...
Assert-LastExitCode "Playwright role-management BL-MVP-031J"
Write-Host "OK: E2E BL-MVP-031 aprobado con EDITOR acotado a la lista de asignaciones."

Restore-GeneratedTypeScriptState

git diff --check
Assert-LastExitCode "git diff --check BL-MVP-031J"
Write-Host "OK: git diff --check aprobado."

Write-Host ""
Write-Host "OK: BL-MVP-031J aplicado y validado."
