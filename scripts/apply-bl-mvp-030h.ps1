...
function Assert-PowerShellSyntax {
    ...
    if ($errors.Count -gt 0) {
        ...
        throw "Error de sintaxis PowerShell en $RelativePath`: $($messages -join ' | ')"
    }
}
...
Write-Host "BL-MVP-030H: corrigiendo sintaxis PowerShell del validador 030G..."

$currentBranch = (git branch --show-current).Trim()
Assert-LastExitCode "Consulta de rama Git"
if ($currentBranch -ne "main") {
    throw "BL-MVP-030H debe aplicarse sobre main. Rama actual: '$currentBranch'."
}
...
Assert-PowerShellSyntax "scripts/apply-bl-mvp-030g.ps1"
Assert-PowerShellSyntax "scripts/apply-bl-mvp-030.ps1"
Assert-PowerShellSyntax "scripts/apply-bl-mvp-030h.ps1"
Write-Host "OK: sintaxis PowerShell de 030, 030G y 030H aprobada."
...
if (-not $scopeHasTarget -or -not $matcherHasTarget) {
    throw "No se encontro AuthorizationScopeKind.Target en ambos archivos."
}
...
Write-Host "OK: correccion CA1720 preserva el scope_type fisico OBJECT."

$baseContent = [System.IO.File]::ReadAllText(
    (Join-Path $RepoRoot "scripts/apply-bl-mvp-030.ps1"),
    [System.Text.Encoding]::UTF8)
...
dotnet format MusicaAprender.sln `
    --verify-no-changes `
    --no-restore
Assert-LastExitCode "dotnet format despues de BL-MVP-030H"
...
dotnet build `
    ...
    --no-restore
Assert-LastExitCode "Build Security BL-MVP-030H"
...
dotnet build `
    ...
    --configuration Release `
    --no-restore
Assert-LastExitCode "Build UnitTests BL-MVP-030H"
...
dotnet test `
    ...
    --no-build `
    --no-restore
Assert-LastExitCode "UnitTests BL-MVP-030H"
Write-Host "OK: pruebas unitarias aprobadas."

Restore-GeneratedTypeScriptState

git diff --check
Assert-LastExitCode "git diff --check BL-MVP-030H"
Write-Host "OK: git diff --check aprobado."

Write-Host ""
Write-Host "OK: BL-MVP-030H aplicado y validado."
