...
Write-Host "BL-MVP-014: creando DbContext y configuraciones EF Core por esquema..."
...
Write-Host "Generando inventario de FK entre propietarios..."
& "$PSScriptRoot/database/generate-cross-schema-foreign-keys.ps1"

Write-Host "Reverse engineering de los nueve esquemas..."
& "$PSScriptRoot/database/scaffold-ef-model.ps1"

$projectPath = "tools\DatabaseModelVerifier\MusicaAprender.DatabaseModelVerifier.csproj"
...
if (-not ($solutionProjects -match [regex]::Escape($projectPath))) {
    ...
    Assert-LastExitCode "Agregar DatabaseModelVerifier a la solucion"
}
...
Write-Host "Formateando C# generado..."
...
Write-Host "Compilando verificador del modelo EF..."
dotnet build $projectPath --no-restore
Assert-LastExitCode "Compilacion DatabaseModelVerifier"

Write-Host "Verificando DbContext/configuraciones contra PostgreSQL..."
...
Write-Host ""
Write-Host "OK: BL-MVP-014 preparado y puerta local aprobada."
Write-Host "Siguiente: .\scripts\local\start.ps1"
Write-Host "Luego:      .\scripts\local\verify-running.ps1"
Write-Host "Despues:    .\scripts\database\verify-ef-model.ps1"
