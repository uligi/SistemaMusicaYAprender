...
Write-Host "BL-MVP-031F: preservando feedback de exito durante la recarga de asignaciones..."

$currentBranch = (git branch --show-current).Trim()
Assert-LastExitCode "Consulta de rama Git"
if ($currentBranch -ne "main") {
    throw "BL-MVP-031F debe aplicarse sobre main. Rama actual: '$currentBranch'."
}
...
foreach ($required in @(
    ...
    "loadAssignments({ clearFeedback: false })",
    "Asignacion aplicada y auditada.",
    "Asignacion retirada y auditada."
)) {
    $normalized = $pageContent.Normalize(
    ...
    if (-not $asciiLike.Contains($required)) {
        throw "RoleManagementPage no contiene el marcador esperado: $required"
    }
}
...
foreach ($artifact in @(
    "INSTRUCCIONES_INSTALACION_MVP_F1_BL-MVP-031F.md",
...
Assert-LastExitCode "Prettier BL-MVP-031F"

& $prettier --check @formatTargets
Assert-LastExitCode "Prettier check BL-MVP-031F"
Write-Host "OK: archivos 031F formateados."

npm.cmd run typecheck
Assert-LastExitCode "TypeScript web BL-MVP-031F"
Write-Host "OK: TypeScript web aprobado."

npm.cmd run typecheck:e2e
Assert-LastExitCode "TypeScript E2E BL-MVP-031F"
...
Assert-LastExitCode "Playwright role-management BL-MVP-031F"
Write-Host "OK: E2E BL-MVP-031 confirma feedback visible, grant y revoke."

Restore-GeneratedTypeScriptState

git diff --check
Assert-LastExitCode "git diff --check BL-MVP-031F"
Write-Host "OK: git diff --check aprobado."

Write-Host ""
Write-Host "OK: BL-MVP-031F aplicado y validado."
