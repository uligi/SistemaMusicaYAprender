...
Write-Host "BL-MVP-031H: usando la respuesta canonica de grant/revoke para actualizar la UI..."

$currentBranch = (git branch --show-current).Trim()
Assert-LastExitCode "Consulta de rama Git"
if ($currentBranch -ne "main") {
    throw "BL-MVP-031H debe aplicarse sobre main. Rama actual: '$currentBranch'."
}
...
foreach ($marker in @(
    ...
    "result.data.assignment.assignmentId",
    "setMessage(",
    "async function loadAssignments(): Promise<void>"
)) {
    if (-not $pageContent.Contains($marker)) {
        throw "RoleManagementPage no contiene el marcador esperado: $marker"
    }
}

if ($pageContent.Contains("loadAssignments({ clearFeedback: false })")) {
    throw "Sigue presente la recarga inmediata posterior a la mutacion."
}
...
foreach ($artifact in @(
    "INSTRUCCIONES_INSTALACION_MVP_F1_BL-MVP-031H.md",
...
Assert-LastExitCode "Prettier BL-MVP-031H"

& $prettier --check @formatTargets
Assert-LastExitCode "Prettier check BL-MVP-031H"
Write-Host "OK: archivos 031H formateados."

npm.cmd run typecheck
Assert-LastExitCode "TypeScript web BL-MVP-031H"
Write-Host "OK: TypeScript web aprobado."

npm.cmd run typecheck:e2e
Assert-LastExitCode "TypeScript E2E BL-MVP-031H"
...
Assert-LastExitCode "Playwright role-management BL-MVP-031H"
Write-Host "OK: E2E BL-MVP-031 confirma grant/revoke y feedback visible."

Restore-GeneratedTypeScriptState

git diff --check
Assert-LastExitCode "git diff --check BL-MVP-031H"
Write-Host "OK: git diff --check aprobado."

Write-Host ""
Write-Host "OK: BL-MVP-031H aplicado y validado."
