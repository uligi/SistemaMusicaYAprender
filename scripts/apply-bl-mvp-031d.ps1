...
Write-Host "BL-MVP-031D: alineando el selector de Rol con su nombre accesible real..."

$currentBranch = (git branch --show-current).Trim()
Assert-LastExitCode "Consulta de rama Git"
if ($currentBranch -ne "main") {
    throw "BL-MVP-031D debe aplicarse sobre main. Rama actual: '$currentBranch'."
}
...
$wrongSelector = "getByLabel('Rol', { exact: true }).selectOption('EDITOR')"
$expectedSelector = "getByLabel('Rol *', { exact: true }).selectOption('EDITOR')"

if ($testContent.Contains($wrongSelector)) {
    throw "El selector 031C que no coincide con el nombre accesible real sigue presente."
}

if (-not $testContent.Contains($expectedSelector)) {
    throw "No se encontro el selector exacto Rol * esperado."
}
...
foreach ($artifact in @(
    "INSTRUCCIONES_INSTALACION_MVP_F1_BL-MVP-031D.md",
...
Assert-LastExitCode "Prettier BL-MVP-031D"

& $prettier --check @formatTargets
Assert-LastExitCode "Prettier check BL-MVP-031D"
Write-Host "OK: archivos 031D formateados."

npm.cmd run typecheck:e2e
Assert-LastExitCode "TypeScript E2E BL-MVP-031D"
...
Assert-LastExitCode "Playwright role-management BL-MVP-031D"
Write-Host "OK: regresion E2E BL-MVP-031 aprobada con el nombre accesible real de Rol."

Restore-GeneratedTypeScriptState

git diff --check
Assert-LastExitCode "git diff --check BL-MVP-031D"
Write-Host "OK: git diff --check aprobado."

Write-Host ""
Write-Host "OK: BL-MVP-031D aplicado y validado."
