...
Write-Host "BL-MVP-030E: corrigiendo Count bajo StrictMode y validando el route-manifest ya migrado..."

$currentBranch = (git branch --show-current).Trim()
Assert-LastExitCode "Consulta de rama Git"
if ($currentBranch -ne "main") {
    throw "BL-MVP-030E debe aplicarse sobre main. Rama actual: '$currentBranch'."
}
...
$canonicalMissing = @(
    @(
        ...
        "CONFIG.MANAGE",
        "SECURITY.READ_AUDIT"
    ) | Where-Object { -not $content.Contains($_) }
)

if ($canonicalMissing.Count -gt 0) {
    throw "Faltan capacidades canonicas: $($canonicalMissing -join ', ')."
}

Write-Host "OK: cero capacidades legacy y catalogo canonico completo."

$baseInstaller = Join-Path $RepoRoot "scripts\apply-bl-mvp-030.ps1"
$installerD = Join-Path $RepoRoot "scripts\apply-bl-mvp-030d.ps1"

foreach ($path in @($baseInstaller, $installerD)) {
...
$baseContent = [System.IO.File]::ReadAllText(
    $baseInstaller,
    [System.Text.Encoding]::UTF8)
$dContent = [System.IO.File]::ReadAllText(
    $installerD,
    [System.Text.Encoding]::UTF8)
...
if (-not $dContent.Contains('$legacy = @(')) {
    throw "El instalador 030D no contiene la coleccion legacy."
}

if (-not $dContent.Contains(') | Where-Object { $content.Contains($_) }')) {
    throw "El instalador 030D no contiene el filtro legacy esperado."
}
...
Assert-LastExitCode "TypeScript BL-MVP-030E"
Write-Host "OK: TypeScript aprobado."

Restore-GeneratedTypeScriptState

git diff --check
Assert-LastExitCode "git diff --check BL-MVP-030E"
Write-Host "OK: git diff --check aprobado."

Write-Host ""
Write-Host "OK: BL-MVP-030E aplicado y validado."
